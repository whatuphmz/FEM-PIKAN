...
class Linear_res:
    ...
    def _forward(self, t: ti.i32, nn_input: ti.template()):
        ...
        if ti.static(self.activation):
            for model_id, k, i in ti.ndrange(self.n_models, self.batch_size, self.n_hidden):
                self.output[model_id, t, k, i] = 0.5*(self.hidden[model_id, t, k, i] + self.bias1[model_id, t, i])*(1+ti.tanh(ti.sqrt(2/3.1415926)*(self.hidden[model_id, t, k, i] + self.bias1[model_id, t, i] + 0.044715*(self.hidden[model_id, t, k, i] + self.bias1[model_id, t, i])**3))) + nn_input[model_id, t, k, i]
        else:
            for model_id, k, i in ti.ndrange(self.n_models, self.batch_size, self.n_hidden):
                self.output[model_id, t, k, i] = self.hidden[model_id, t, k, i] + self.bias1[model_id, t, i]
